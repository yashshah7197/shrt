// Package checkgroup maintains the group of handlers for health checking.
package checkgroup
...
// Readiness checks if the database is ready and if not, will return a 500 status. Do not respond
// by just returning an error because further up in the call stack it will interpret that as a
// non-trusted error.
...
// Kubernetes cluster, it will also return pod, node and namespace details via the Downward API.
